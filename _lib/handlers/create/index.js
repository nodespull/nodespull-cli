...
const testEnv_1 = __importDefault(require("./templates/loaders/testEnv"));
const testConfig_1 = __importDefault(require("./templates/loaders/testConfig"));
let projectName;
if (!process.argv[3]) {
...
function builde2eDir() {
    cmd_1.default("mkdir", ["-p", "e2e/environment"]);
    cmd_1.default("touch", ["e2e/environment/test.local.env.js"]);
    cmd_1.default("touch", ["e2e/environment/test.prod.env.js"]);
    fs_1.default.writeFileSync("./e2e/environment/test.local.env.js", testEnv_1.default("local"));
    fs_1.default.writeFileSync("./e2e/environment/test.local.env.js", testEnv_1.default("production"));
    cmd_1.default("mkdir", ["-p", "e2e/resources/reports"]);
    cmd_1.default("mkdir", ["-p", "e2e/resources/utils"]);
    cmd_1.default("mkdir", ["-p", "e2e/spec"]);
    cmd_1.default("mkdir", ["-p", "e2e/validation/common"]);
    cmd_1.default("touch", ["e2e/config.json"]);
    fs_1.default.writeFileSync("./e2e/config.json", testConfig_1.default());
}
function buildConfigDir() {
    cmd_1.default("mkdir", ["-p", "config"]);
    cmd_1.default("touch", ["./config/secrets.local.env"]);
    cmd_1.default("touch", ["./config/secrets.test.env"]);
    cmd_1.default("touch", ["./config/readme.md"]);
}
